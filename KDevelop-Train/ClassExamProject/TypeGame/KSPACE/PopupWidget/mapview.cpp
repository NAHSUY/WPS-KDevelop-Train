...
#include <QPainter>         // 新增
#include <QGraphicsScene>   // 新增
#include <QGraphicsView>    // 新增
#include <QGraphicsPixmapItem> // 新增
#include <utility>          // 新增 (为了 std::as_const)
...
{
	bool isInputValid;
	// 去掉 QStringLiteral，且字符串不要换行断开！
	QString nickname = QInputDialog::getText(this, QString("游戏得分记录"), QString("请输入你的昵称："), QLineEdit::Normal, QString("玩家"), &isInputValid);

	if (isInputValid && !nickname.isEmpty() && nickname != QString("玩家"))
	{
		return nickname;
	}
	else
	{
		return QString("未输入名称");
	}
}
