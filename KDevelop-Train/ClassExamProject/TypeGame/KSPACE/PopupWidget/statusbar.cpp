#pragma execution_character_set("utf-8") // 修改1：镇压 MSVC 中文换行乱码的究极神咒
#include "statusbar.h"
#include "public_enum.h"
#include <QFile>
#include <QPainter>

const QFont StatusBar::fontSmall = QFont("Arial", 10);

const QPixmap& StatusBar::pixStatusBar()
{
	static const QPixmap pix(":/imagePng/PNG/POLICE_INPUT.png");
	return pix; // 修改2：补上了必须的 return
}
...
void StatusBar::initLabels()
{
	...
	this->information = new QLabel("难度即将上升", this); // 有了上面的 utf-8 神咒，这里绝对不会再报错
	...
	scoreString->setText("0"); // 修改3：不能传数字0，必须传字符串 "0"
...
void StatusBar::init()
{
	...
	this->remainingTime = 120;
	this->scoreString->setText("0"); // 修改4：不能传数字0，必须传字符串 "0"
}
