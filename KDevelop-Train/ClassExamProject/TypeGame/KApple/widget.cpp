...
{
	// 将配置文件定位到应用程序可执行文件同目录，避免 VS 的工作目录差异
	QString configPath = QCoreApplication::applicationDirPath() + "/KAppleConfig.ini";
	QSettings setting(configPath, QSettings::IniFormat);

	// 使用类成员默认值作为后备值，防止键不存在导致 0
	this->m_gameLevel = setting.value("GameLevel", this->m_gameLevel).toInt();
	this->m_failNum = setting.value("FailNum", this->m_failNum).toInt();
	this->m_winNum = setting.value("WinNum", this->m_winNum).toInt();

	// 范围校验，避免配置错误导致立刻胜利或失败
	if (this->m_winNum <= 0) this->m_winNum = 100;
	if (this->m_failNum <= 0) this->m_failNum = 5;
	if (this->m_gameLevel < 1 || this->m_gameLevel > 9) this->m_gameLevel = 3;
